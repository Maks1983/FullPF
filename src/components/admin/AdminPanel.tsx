...
const StepUpVerificationCard: React.FC = () => {
  const { verifyStepUp, isStepUpValid, stepUpState, STEP_UP_CODE } =
    useAdminFoundation();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setStatus("idle");
    setMessage("");
    const success = await verifyStepUp("impersonation", code.trim());
    if (success) {
      setStatus("success");
      setMessage(
        "Step-up verification confirmed for the next sensitive action.",
      );
      setCode("");
    } else {
      setStatus("error");
    ...
    <div className="bg-white border border-purple-200 rounded-2xl p-6 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-start gap-3">
          <ShieldCheck
            className={`h-6 w-6 ${verified ? "text-emerald-500" : "text-purple-500"}`}
          />
          <div>
            ...
            <p className="text-sm text-gray-600">
              Sensitive actions (impersonation, config edits, restores,
              deletions) require a recent verification. Use the demo TOTP code{" "}
        ...
        <div
          className={`px-3 py-1 text-sm font-medium rounded-full ${verified ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
        >
      ...
      {stepUpState.lastVerifiedAt && (
        <p className="mt-2 text-xs text-gray-500">
          Last verified at {formatTimestamp(stepUpState.lastVerifiedAt)} (valid
          for 5 minutes).
        </p>
      )}
    </div>
...
const ImpersonationSection: React.FC = () => {
  ...
  const handleImpersonate = async (user: DirectoryUser) => {
    ...
    const result = await startImpersonation(user.id, "support-session");
    if (result.success) {
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      setFeedback("Impersonation session cleared.");
    } catch (error) {
      setError(
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Instance-level toggles. Owner overrides only; no changes allowed
            while impersonating.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {Object.values(featureFlags).map((flag) => {
          const canToggle =
            flag.overridableBy.includes(session?.role ?? "readonly") &&
            !impersonation;
            ...
            >
              <div>
                ...
                <div className="mt-1 text-xs text-gray-500 flex flex-wrap items-center gap-2">
                  <span>
                    Last updated {formatTimestamp(flag.lastChangedAt)}
                  </span>
              ...
              <label className="inline-flex items-center gap-3">
                <input
                  ...
                  checked={flag.value}
                  onChange={(event) => {
                    void handleToggle(flag.key, event.target.checked);
                  }}
                  disabled={!canToggle}
                />
                <span
                  className={`text-sm ${canToggle ? "text-gray-700" : "text-gray-400"}`}
                >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              All admin-sensitive actions are recorded, including impersonation
              context.
            </p>
          </div>
        </div>
...
const ConfigSection: React.FC = () => {
  const { configItems, updateConfigItem, canModifyConfig, isStepUpValid } =
    useAdminFoundation();
  ...
  const handleSave = async (itemKey: string) => {
    setFeedback(null);
    setError(null);
    if (!isStepUpValid()) {
    ...
    const result = await updateConfigItem(itemKey, draftValue, {
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Stored securely in encrypted storage. Editing requires Owner access
            plus step-up verification.
          </p>
        </div>
      </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    {item.encrypted ? "Encrypted" : "Plaintext"} •{" "}
                    {item.requiresStepUp ? "Step-up required" : "Basic edit"}
                    {item.lastUpdatedAt && (
                      <span className="ml-2">
                        Last updated {formatTimestamp(item.lastUpdatedAt)}
                      </span>
                    )}
                  </p>
                </div>
              ...
              {isEditing ? (
                <div className="mt-4 space-y-3">
                  <textarea
                    ...
                    placeholder={
                      item.masked ? "Enter new secret value" : undefined
                    }
...
const InfrastructureSection: React.FC = () => {
  ...
  const handleBackup = async (mode: "manual" | "scheduled") => {
    setBackupMessage(null);
    setBackupError(null);
    const result = await triggerBackup(mode);
    if (result.success) {
  ...
  const handleDeletion = async () => {
    ...
    if (!isStepUpValid()) {
      setDeletionError(
        "Step-up verification required before scheduling deletion.",
      );
      return;
    }
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Run backups, restore snapshots, and manage full instance deletion
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="rounded-xl border border-gray-100 p-4 shadow-sm">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Last backup: {formatTimestamp(infrastructure.lastBackupAt)} (
            {infrastructure.lastBackupMode ?? "n/a"}).
          </p>
          {(backupMessage || backupError) && (
            <div
              className={`mt-3 rounded-lg px-3 py-2 text-xs ${backupMessage ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
            >
              {backupMessage ?? backupError}
            </div>
          )}
        ...
        <div className="rounded-xl border border-gray-100 p-4 shadow-sm">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Dry-run changes metadata only; full restore requires step-up and is
            owner-only.
          </p>
          <input
            value={restoreId}
          ...
          {(restoreMessage || restoreError) && (
            <div
              className={`mt-3 rounded-lg px-3 py-2 text-xs ${restoreMessage ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
            >
              {restoreMessage ?? restoreError}
            </div>
          )}
        ...
        <div className="rounded-xl border border-gray-100 p-4 shadow-sm">
          ...
          <p className="mt-2 text-sm text-gray-600">
            ...
            and confirm after step-up. Destructive actions disabled while
            impersonating.
          </p>
          <input
          ...
          {(deletionMessage || deletionError) && (
            <div
              className={`mt-3 rounded-lg px-3 py-2 text-xs ${deletionMessage ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
            >
              {deletionMessage ?? deletionError}
            </div>
          )}
          ...
          {infrastructure.deletionScheduledFor && (
            <div className="mt-3 space-y-2 rounded-xl bg-rose-50 p-3 text-xs text-rose-600">
              <div>
                ...
                {formatTimestamp(infrastructure.deletionScheduledFor)} on{" "}
                {instanceName}.
              </div>
              <button
                onClick={() => {
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <MonitoringCard
          ...
          status={
            monitoring.dbConnection === "healthy"
              ? "Online"
              : monitoring.dbConnection
          }
          tone={
            monitoring.dbConnection === "healthy"
              ? "success"
              : monitoring.dbConnection === "degraded"
                ? "warning"
                : "critical"
          }
        />
        <MonitoringCard
          ...
          tone={
            monitoring.smtpStatus === "ok"
              ? "success"
              : monitoring.smtpStatus === "warning"
                ? "warning"
                : "critical"
          }
        />
        <MonitoringCard
          ...
          tone={
            monitoring.cpuUtilization < 70
              ? "success"
              : monitoring.cpuUtilization < 85
                ? "warning"
                : "critical"
          }
        />
        <MonitoringCard
          ...
          tone={
            monitoring.memoryUtilization < 70
              ? "success"
              : monitoring.memoryUtilization < 85
                ? "warning"
                : "critical"
          }
        />
        <MonitoringCard
          ...
          tone={
            monitoring.queueBacklog === 0
              ? "success"
              : monitoring.queueBacklog < 5
                ? "warning"
                : "critical"
          }
        />
...
const LicenseSection: React.FC = () => {
  const { license, overrideLicenseTier, canRunCriticalOps, tierFeatures } =
    useAdminFoundation();
  const [overrideSelection, setOverrideSelection] = useState<string>(
    license.overrideActive ? (license.overrideTier ?? "") : "none",
  );
  const [message, setMessage] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const handleOverride = async (value: string) => {
    setMessage(null);
    setError(null);
    setOverrideSelection(value);
    const tier = value === "none" ? null : (value as LicenseTier);
    const result = await overrideLicenseTier(tier);
    if (result.success) {
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">
            Local validation for demo licenses. Owner can temporarily override
            tiers for support scenarios.
          </p>
        </div>
      </div>
      ...
      {(message || error) && (
        <div
          className={`rounded-xl px-3 py-2 text-sm ${message ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}
        >
          {message ?? error}
        </div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-gray-100 p-4 shadow-sm">
          ...
          <p className="text-sm text-gray-600">
            Tier: {tierLabels[tierToDisplay]}
          </p>
          <p className="text-sm text-gray-600">
            Expires: {formatTimestamp(license.expiresAt)}
          </p>
          <p className="text-sm text-gray-600">
            Last validated: {formatTimestamp(license.lastValidatedAt)}
          </p>
          <span
            className={`inline-flex mt-2 items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold ${statusBadge}`}
          >
        ...
        <div className="rounded-xl border border-gray-100 p-4 shadow-sm">
          ...
          <p className="mt-2 text-sm text-gray-600">
            Use for debug/support after verifying with step-up. Overrides only
            apply while the Owner is not impersonating.
          </p>
          <select
          ...
          {Object.entries(tierFeatures).map(([tier, features]) => (
            <div key={tier} className="rounded-lg bg-gray-50 p-3">
              <div className="font-semibold text-gray-800">
                {tierLabels[tier as LicenseTier]}
              </div>
...
const AdminHeader: React.FC = () => {
  const { session, effectiveSession, impersonation, canAccessAdmin } =
    useAdminFoundation();
    ...
    <div className="rounded-3xl bg-gradient-to-r from-purple-600 via-purple-500 to-indigo-600 p-6 text-white shadow-lg">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-purple-100">
            Manage roles, premium access, infrastructure safeguards, and audit
          ...
          {impersonation && (
            <div className="rounded-2xl bg-white/20 px-4 py-2">
              ...
              <p className="text-sm font-semibold">
                {impersonation.target.displayName}
              </p>
              <p className="text-xs text-purple-100">
                {roleLabels[impersonation.target.role]}
              </p>
            </div>
          )}
        </div>
      ...
      {impersonation && (
        <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white/15 px-3 py-2 text-sm">
          ...
          You are impersonating {impersonation.target.displayName}. All actions
      ...
      {!impersonation &&
        effectiveSession &&
        session &&
        effectiveSession.id !== session.id && (
          <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white/15 px-3 py-2 text-sm">
  ...
  <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-700">
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-sm">
          Only Owner and Manager roles can open the admin console.
        </p>
      </div>
    </div>
  </div>
