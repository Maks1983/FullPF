      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
                ...
                <div key={account.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span className={`px-2 py-1 text-xs rounded-full ${
                      ...
                      account.status === 'overdrawn' ? 'bg-red-100 text-red-800' :
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      {account.type === 'credit' && account.creditLimit && (
                        <>
                          <div>
                            ...
                            <span className="font-medium ml-2">NOK {account.creditLimit.toLocaleString()}</span>
                          </div>
                          <div>
                      ...
                      {account.minimumBalance && (
                        <div>
                          ...
                          <span className="font-medium ml-2">NOK {account.minimumBalance.toLocaleString()}</span>
                        </div>
                      )}
                      {account.overdraftLimit && (
                        <div>
                          <span className="text-gray-600">Overdraft:</span>
                          <span className="font-medium ml-2">NOK {account.overdraftLimit.toLocaleString()}</span>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-between items-center">
          <div className="text-sm text-gray-600">
            {accounts.length} account{accounts.length !== 1 ? 's' : ''} • Total: NOK {totalBalance.toLocaleString()}
          </div>
