...
router.post('/login', strictRateLimit, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [userRows] = await db.query(
      `SELECT u.*, l.tier, l.status as license_status
       FROM users u
    ...
    await db.query(
      `INSERT INTO sessions (id, user_id, refresh_token, expires_at, ip_address, user_agent)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [uuidv4(), user.id, refreshToken, refreshExpiresAt, req.ip, req.get('user-agent') || null]
    );

    // Log successful login
    await db.query(
      `INSERT INTO audit_logs (id, user_id, action, entity_type, entity_id, changes, ip_address, user_agent)
       VALUES (?, ?, 'login', 'user', ?, ?, ?, ?)`,
      [uuidv4(), user.id, user.id, JSON.stringify({ success: true }), req.ip, req.get('user-agent') || null]
    );
...
router.post('/refresh', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Fetch user with license info
    const [userRows] = await db.query(
      `SELECT u.id, u.email, u.role, l.tier
       FROM users u
       LEFT JOIN licenses l ON u.license_id = l.license_id
  ...
  try {
    ...
    if (req.user) {
      await db.query(
        `INSERT INTO audit_logs (id, user_id, action, entity_type, entity_id, changes, ip_address)
         VALUES (?, ?, 'logout', 'user', ?, ?, ?)`,
        [uuidv4(), req.user.id, req.user.id, JSON.stringify({ success: true }), req.ip]
      );
    }
