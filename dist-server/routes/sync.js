...
router.get('/status', async (req, res) => {
    try {
        ...
        const [connectionRows] = await db.query(`SELECT
        id,
        provider,
      ...
      WHERE user_id = ?
      ORDER BY institution_name`, [userId]);
        // Get account count per connection
        const connections = await Promise.all(connectionRows.map(async (conn) => {
            const [accountRows] = await db.query(`SELECT COUNT(*) as count
           FROM accounts
           WHERE user_id = ? AND JSON_EXTRACT(metadata, '$.connection_id') = ?`, [userId, conn.id]);
        ...
        const needsSync = connections.filter((c) => c.status === 'active' && (!c.lastSyncedAt || isStale(c.lastSyncedAt))).length;
 ...
 */
function getLatestSyncTime(connections) {
