...
router.get('/', async (req, res) => {
    try {
        ...
        const { accountId, startDate, endDate, category, limit = '100', offset = '0', } = req.query;
        let query = `
      SELECT
        t.id,
        ...
        try {
            for (const txn of transactions) {
                try {
                    ...
                    const [accountRows] = await connection.query('SELECT id FROM accounts WHERE id = ? AND user_id = ?', [txn.accountId, userId]);
                    ...
                    let existingTxnId = null;
                    if (txn.metadata?.external_id) {
                        const [existing] = await connection.query(`SELECT id FROM transactions
               WHERE user_id = ? AND JSON_EXTRACT(metadata, '$.external_id') = ?`, [userId, txn.metadata.external_id]);
                        if (Array.isArray(existing) && existing.length > 0) {
                            existingTxnId = existing[0].id;
                        }
                    }
                    if (existingTxnId) {
                        // Update existing transaction
                        await connection.query(`UPDATE transactions SET
                account_id = ?,
                transaction_date = ?,
                description = ?,
                amount = ?,
               ...
               WHERE id = ? AND user_id = ?`, [
                ...
                tags, metadata
                    ...
                    await connection.query(`UPDATE accounts SET
              balance = balance + ?,
              updated_at = NOW()
             WHERE id = ? AND user_id = ?`, [txn.amount, txn.accountId, userId]);
                }
                catch (error) {
                    results.errors.push({
                        transaction: txn,
